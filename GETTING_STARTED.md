...
+-- tools/                                            # synclite-{client,db,dbreader,qreader,jobmonitor,consolidator,sample-app}
+-- sample-apps/                                      # cpp / java / python / rust (one sample per language)
```
...
2. Deploy `synclite-db-1.0.0.war` (bundled under `tools/synclite-db/`) to Tomcat and configure its device to log to the **same** staging location.
...
> Use [http://localhost:8080/synclite-jobmonitor](http://localhost:8080/synclite-jobmonitor) to watch both the producer and the Consolidator jobs, their progress, and any errors in one place.
...
1. **Deploy the WAR:**  
   - Copy `synclite-db-1.0.0.war` (from `tools/synclite-db/` or `root/web/target/`) into the `webapps/` directory of your Apache Tomcat server.
