...
REM =========================================================================
REM Helper: Extract ZIP archives
REM =========================================================================
:extract_zip
...
if defined PWSH_EXE (
	"!PWSH_EXE!" -NoProfile -Command ^
		"try { Expand-Archive -Force -Path '%ZIP_PATH%' -DestinationPath '%DEST_PATH%'; exit 0 } catch { exit 1 }"
	exit /b !ERRORLEVEL!
)
...
if defined PWSH_EXE (
	"!PWSH_EXE!" -NoProfile -Command ^
		"try { Copy-Item -Path '%~1\\*' -Destination '%~2' -Recurse -Force; exit 0 } catch { exit 1 }"
	exit /b !ERRORLEVEL!
)
...
if exist "%TOMCAT_DIR%\conf\server.xml" if exist "%TOMCAT_DIR%\bin\catalina.bat" (
	set "TOMCAT_READY=1"
)
...
if defined TOMCAT_READY (
	...
	if exist "%TOMCAT_ZIP%" (
		...
		if errorlevel 1 (
			echo !ERR!ERROR: Failed to download Tomcat.!RESET!
			call :hold_window failure
			exit /b 1
		)
	...
	if errorlevel 1 (
		echo !ERR!ERROR: Failed to extract Tomcat.!RESET!
		call :hold_window failure
		exit /b 1
	)
...
REM =========================================================================
REM Download and extract OpenJDK
REM =========================================================================
...
if defined JDK_READY (
	...
	if exist "%JDK_ZIP%" (
		...
		if errorlevel 1 (
			echo !ERR!ERROR: Failed to download OpenJDK.!RESET!
			call :hold_window failure
			exit /b 1
		)
	...
	if errorlevel 1 (
		echo !ERR!ERROR: Failed to extract OpenJDK.!RESET!
		call :hold_window failure
		exit /b 1
	)
	...
	if exist "%JDK_DIR%" (
		...
		if exist "%JDK_DIR%" (
			echo !ERR!ERROR: Failed to remove existing %JDK_DIR%.!RESET!
			call :hold_window failure
			exit /b 1
		)
	)
	...
	for /d %%D in (jdk_tmp\jdk-*) do (
		...
		if errorlevel 1 (
			...
			if errorlevel 1 (
				echo !ERR!ERROR: Failed to finalize JDK installation.!RESET!
				call :hold_window failure
				exit /b 1
			)
		)
	...
	if not defined JDK_MOVED (
		echo !ERR!ERROR: Extracted JDK folder not found.!RESET!
		call :hold_window failure
		exit /b 1
	)

	if not exist "%JDK_DIR%\bin\java.exe" (
		echo !ERR!ERROR: JDK setup failed.!RESET!
		call :hold_window failure
		exit /b 1
	)
...
set WEBAPPS=%TOMCAT_DIR%\webapps

echo !STEP![7/7] Deploying WAR files to Tomcat...!RESET!

call :deploy_war "SyncLite Consolidator" "synclite-consolidator" "Consolidator WAR not found in packaged or module build output." "..\tools\synclite-consolidator\synclite-consolidator-*.war" "..\tools\synclite-consolidator\*.war" "..\target\synclite-platform-1.1.0\tools\synclite-consolidator\synclite-consolidator-*.war" "..\synclite-consolidator\root\web\target\synclite-consolidator-*.war" "..\synclite-consolidator\root\web\target\*.war"
if errorlevel 1 exit /b 1

call :deploy_war "SyncLite Sample App" "synclite-sample-app" "Sample app WAR not found in packaged or module build output." "..\tools\synclite-sample-app\*.war" "..\target\synclite-platform-1.1.0\tools\synclite-sample-app\*.war" "..\synclite-sample-web-app\web\target\*.war"
if errorlevel 1 exit /b 1

call :deploy_war "SyncLite DB" "synclite-db" "SyncLite DB WAR not found in packaged or module build output." "..\tools\synclite-db\*.war" "..\target\synclite-platform-1.1.0\tools\synclite-db\*.war" "..\synclite-db\root\web\target\synclite-db-*.war" "..\synclite-db\root\web\target\*.war"
if errorlevel 1 exit /b 1

call :deploy_war "SyncLite DBReader" "synclite-dbreader" "DBReader WAR not found in packaged or module build output." "..\tools\synclite-dbreader\*.war" "..\target\synclite-platform-1.1.0\tools\synclite-dbreader\*.war" "..\synclite-dbreader\root\web\target\synclite-dbreader-*.war" "..\synclite-dbreader\root\web\target\*.war"
if errorlevel 1 exit /b 1

call :deploy_war "SyncLite QReader" "synclite-qreader" "QReader WAR not found in packaged or module build output." "..\tools\synclite-qreader\*.war" "..\target\synclite-platform-1.1.0\tools\synclite-qreader\*.war" "..\synclite-qreader\root\web\target\synclite-qreader-*.war" "..\synclite-qreader\root\web\target\*.war"
if errorlevel 1 exit /b 1

call :deploy_war "SyncLite Job Monitor" "synclite-jobmonitor" "Job Monitor WAR not found in packaged or module build output." "..\tools\synclite-jobmonitor\*.war" "..\target\synclite-platform-1.1.0\tools\synclite-jobmonitor\*.war" "..\synclite-job-monitor\root\web\target\synclite-jobmonitor-*.war" "..\synclite-job-monitor\root\web\target\*.war"
if errorlevel 1 exit /b 1
